...
encoder = 'vitl'    # option for model variant
...
def quantize_to_int8_dynamic(model):

    print("Applying dynamic quantization...")
    model_quantized = torch.quantization.quantize_dynamic(
        model,  # Model to quantize
        {torch.nn.Linear, torch.nn.Conv2d},  # Layers to quantize dynamically
        dtype=torch.qint8  # Use 8-bit integers
    )
    return model_quantized

# Quantize the model
model_quantized = quantize_to_int8_dynamic(model)

# Save the quantized model
quantized_checkpoint_path = f'checkpoints/depth_anything_v2_{encoder}_dynamicquantized_int8.pth'
torch.save(model_quantized.state_dict(), quantized_checkpoint_path)
print(f"Quantized INT8 model saved to {quantized_checkpoint_path}")
...
# Results
print("\nMetrics for Quantized Model (INT8):")
